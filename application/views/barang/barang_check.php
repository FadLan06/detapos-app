...
<script type="text/javascript">
    ...
    $(document).ready(function() {
        $('#mLink').on('show.bs.modal', function(e) {
            var kd = $(e.relatedTarget).data('id');
            //menggunakan fungsi ajax untuk pengambilan data
            $.ajax({
                type: 'post',
                url: '<?= base_url('Barang/link_check') ?>',
                data: 'id=' + kd,
                success: function(data) {
                    $('._link_check').html(data); //menampilkan data ke dalam modal
                }
            });
        });
    });
...
<script>
    ...
    function ambilBarangC() {
        $.ajax({
            type: 'ajax',
            url: '<?php echo base_url() ?>Barang/get_barangC',
            async: false,
            dataType: 'json',
            success: function(data) {
                var html = '';
                ...
                for (i = 0; i < data.length; i++) {
                    $link = '<a href="" class="btn btn-secondary btn-sm <?= $akses->ubah != 1 ? 'disabled' : '' ?>" data-target="#mLink" data-toggle="modal" data-id="' + data[i].id + '"><i class="fas fa-link"></i> Link</a>';
                    $returc = '<a href="" class="btn btn-warning btn-sm" data-target="#mRetur" data-toggle="modal" data-id="' + data[i].id + '"><i class="fas fa-undo"></i> Retur</a>';
                    $aksic = '<a href="<?= base_url('Barang/Ubah_Barang/') ?>' + data[i].id + '"><i class="fas fa-edit"></i></a> <a href="<?= base_url('Barang/Hapus_brng/') ?>' + data[i].id + '/' + data[i].kode_barang + '" onclick="return confirm("Yakin anda ?")"><i class="fas fa-trash text-danger"></i></a>';
                    ...
                    html += '<tr>' +
                        ...
                        '<td width="10%" style="text-align: center;">' + $link + '</td>' +
                        '<td width="10%" style="text-align: center;">' + $returc + '</td>' +
                        '<td style="text-align:center; width:10%">' + $aksic + '</td>' +
                        '</tr>';
                }
                $('#data_barangC').html(html);

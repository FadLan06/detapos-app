                                ...
                                <li class="list-inline-item dropdown notification-list">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-arrow dropdown-menu-lg">
                                        ...
                                        <?php else : ?>
                                            ...
                                            <?php foreach ($this->cart->contents() as $items) : ?>
                                                <?php $barang = $this->db->get_where('tb_barang', ['gambar !=' => null, 'id' => $items['id']])->row_array(); ?>
                                                <!-- item-->
                                                <a href="javascript:void(0);" class="dropdown-item notify-item">
                                                    <div class="notify-icon"><img src="<?= base_url() ?>assets/upload/barang/<?= $barang['gambar'] ?>" alt="user-img" class="img-fluid" /> </div>
                                                    <p class="notify-details"><b><?= $items['name'] ?></b><small class="text-muted"><?= $items['qty'] ?> x <?= number_format($items['price']) ?> = Rp. <?= number_format($items['subtotal']) ?></small></p>
                                                </a>
                                            <?php endforeach; ?>
                                            ...
                                            <div class="row">
                                                <div class="col-md-6">
                                                    <button type="button" onclick="location.href='<?= base_url('Olshop/') . $this->uri->segment(2) . '/cart' ?>'" class="btn btn-block btn-sm btn-warning" style="width:85%; margin:auto">
                                                        View Cart
                                                    </button></div>
                                                <div class="col-md-6">
                                                    <a href="javascript:void(0);" onclick="location.href='<?= base_url('Olshop/') . $this->uri->segment(2) . '/check' ?>'" class="btn btn-block btn-sm btn-deta" style="width:85%; margin:auto">
                                                        Checkout
                                                    </a>
                                                </div>
                                            </div>

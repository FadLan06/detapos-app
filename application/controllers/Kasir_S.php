...
class Kasir_S extends CI_Controller
{
    ...
    public function index()
    {
        ...
        $data['no_kwitansi'] = $this->Data_model->no_kwitansi1();
        $data['no_jurnal'] = $this->Akuntansi_model->no_jurnal();

        $this->load->view('templates/header', $data);
        $this->load->view('transaksi/kasir_s', $data);
    ...
    function simpan_barang()
    {
        ...
        $data = $this->Data_model->simpan_barang($kobar, $nabar, $harga, $harga_beli, $potongan, $petugas, $qty, $timee, $token);
    ...
    function data_kasir()
    {
        $data['user'] = $this->db->get_where('user', ['username' => $this->session->userdata('username')])->row_array();
        $token = $data['user']['token'];
        $data = $this->db->query("SELECT *, sum(harga * qty - potongan) as sub_total, sum(qty) as kty, sum(potongan) as potongan FROM tb_detail_penjualan_tmp WHERE token='$token' GROUP BY harga, kode_barang ORDER BY kode_barang")->result();
        echo json_encode($data);
    }
    ...
    public function smpn_pen()
    {
        if (isset($_POST['enter'])) {
            ...
            $dt = $this->db->query("SELECT * FROM tb_detail_penjualan_tmp WHERE token='" . $this->session->userdata('token') . "'");
            $zona = $this->db->get_where('setting_app', ['token' => $this->session->userdata('token')])->row_array();
            date_default_timezone_set($zona['zona']);
            if ($dt->num_rows() > 0) {
                ...
                redirect('Kasir_Retail/Cetak_Struk/' . $no_transaksi);
            }
        }
    }
    ...
    public function cetak_struk($id)
    {
        ...
        if ($akses3['menu_id'] = 40) {
            $data['penjualan'] = $this->db->query("SELECT *, sum(harga*qty) as sub_total, sum(qty) as kty FROM tb_detail_penjualan WHERE no_transaksi='$id' AND token='$token' GROUP BY harga, kode_barang ORDER BY kode_barang")->result_array();
        } else {
